...
class MultiscaleNetModel(chainer.Chain):
    ...
    def __init__(self, n_class):
        ...
        # Deep layers
        self._layers["conv1"] = L.Convolution2D(3, 64, 7, stride=2, pad=3, **kwargs),
        self._layers["conv2_reduce"] = L.Convolution2D(64, 64, 1, **kwargs),
        self._layers["conv2"] = L.Convolution2D(64, 192, 3, stride=1, pad=1, **kwargs),
        self._layers["inc3a"] = L.Inception(192, 64, 96, 128, 16, 32, 32),
        self._layers["inc3b"] = L.Inception(256, 128, 128, 192, 32, 96, 64),
        self._layers["inc4a"] = L.Inception(480, 192, 96, 208, 16, 48, 64),
        self._layers["inc4b"] = L.Inception(512, 128, 128, 256, 24, 64, 64),
        self._layers["inc4d"] = L.Inception(512, 112, 144, 288, 32, 64, 64),
        self._layers["inc4e"] = L.Inception(528, 256, 160, 320, 32, 128, 128),
        self._layers["inc5a"] = L.Inception(832, 256, 160, 320, 32, 128, 128),
        self._layers["inc5b"] = L.Inception(832, 384, 192, 384, 48, 128, 128),
        self._layers["loss3_fc"] = L.Linear(1024, 1000, **kwargs),

        self._layers["loss1_conv"] = L.Convolution2D(512, 128, 1, **kwargs),
        self._layers["loss1_fc1"] = L.Linear(2048, 1024, **kwargs),
        self._layers["loss1_fc2"] = L.Linear(1024, 1000, **kwargs),

        self._layers["loss2_conv"] = L.Convolution2D(528, 128, 1, **kwargs),
        self._layers["loss2_fc1"] = L.Linear(2048, 1024, **kwargs),
        self._layers["loss2_fc2"] = L.Linear(1024, 1000, **kwargs)
